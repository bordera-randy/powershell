...
function New-StarButton {
    ...
    # Use unicode stars to avoid font/glyph issues on older systems
    if (Get-IsFavorite -Name $CenterName) { $b.Content = "★" } else { $b.Content = "☆" }
    $b.FontSize = 16
    $b.FontWeight = "Bold"
...
function Update-AllStarIcons {
    foreach ($entry in $buttonMap) {
        if ($null -ne $entry.StarButton) {
            if (Get-IsFavorite -Name $entry.Name) { $entry.StarButton.Content = "★" } else { $entry.StarButton.Content = "☆" }
        }
    }
    foreach ($entry in $favButtons) {
        if ($null -ne $entry.StarButton) {
            # favorites row is always favorite; show filled star
            $entry.StarButton.Content = "★"
        }
    }
}
...
function Build-Favorites {
    ...
    foreach ($pinName in $pins) {
        ...
        $grid.Margin = "0"
        $grid.Children.Add((New-StarButton -CenterName $center.Name)) | Out-Null
        ...
        $favButtons.Add([pscustomobject]@{ Button=$btn; Name=$center.Name; Keywords=(Nz $center.Keywords); StarButton=($grid.Children[0]) }) | Out-Null
    }
}
